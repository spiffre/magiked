...
export class Walker
{
	...
	async readFile (name: string, filepath: string, parent: DirectoryNode): Promise<FileNode>
	{
		...
		{
			fileNode.payload = await this.parseImportExportsFromFile(filepath)
		}
	...
	private async parseImportExportsFromFile (filepath: string): Promise<Payload>
	{
		const content = await readFileAsync(filepath)
		const contentAsStr = new TextDecoder("utf-8").decode(content)
		const rootAst = espree.parse(contentAsStr, ESPREE_PARSE_OPTIONS)
		...
		for (const topLevelNode of rootAst.body)
		{
			const importNode = this.createImportNode(topLevelNode, scope)
			if (importNode)
			{
				payload.imports.push(importNode)
			}
			
			const exportNode = this.createExportNode(topLevelNode, scope)
			if (exportNode)
			{
				if (exportNode.type == 'ExportMetaAst')
				{
					payload.exports.push(exportNode)
				}
				else
				{
					payload.reexports.push(exportNode)
				}
			}
		}
	...
	private createImportNode (
		node: EspreeAst.Directive | EspreeAst.Statement | EspreeAst.ModuleDeclaration,
		scope: string
	): ImportMetaAst | null
	{
		if (node.type == 'VariableDeclaration')
		{
			assert(node.declarations[0])
				
			const declaration = node.declarations[0]
			
			// Early out if there's no initializer
			if (declaration.init == null)
			{
				return null
			}
			
			// Looks like a require() call follow by an accessor
			if (declaration.init.type == 'MemberExpression')
			{
				// Check that we have: require(<SomeString>).<SomeMember>
				if (
					declaration.init.object.type == 'CallExpression' &&
					declaration.init.object.callee.type == 'Identifier' &&
					declaration.init.object.callee.name == 'require'
				)
				{
					const identifier = declaration.id
					const requireCallArgument = declaration.init.object.arguments[0]
					const importedSymbol = declaration.init.property
					
					if (
						identifier.type == "Identifier" &&
						requireCallArgument.type == "Literal" &&
						importedSymbol.type == "Identifier"
					)
					{
						const localName = identifier.name
						assert(typeof localName == 'string')
						
						const importPath = requireCallArgument.value
						assert(typeof importPath == 'string')
						
						const importedSymbolName = importedSymbol.name
						assert(typeof importedSymbolName == 'string')
						
						const loc =
						{
							// The begining is the start of the variable declaration
							start : ( node as WithLocation<EspreeAst.VariableDeclaration> ).start,
							
							// The end is after the end of the property in the MemberExpression
							end : ( declaration.init.property as WithLocation<EspreeAst.PrivateIdentifier> ).end
						}
						
						// 2 options: the resource is a package name or a relative path to a local file
						const isLocal = importPath[0] == '.'
						if (isLocal)
						{
							return {
								type : "ImportMetaAst",
								kind : "File",
								path : this.normalizedPath(importPath, scope),
								loc,
								
								localId : localName,
								symbolId : importedSymbolName
							}
						}
						else
						{
			...
			// Looks like a straight up require() call
			else if (declaration.init.type == 'CallExpression')
			{
				// Check that we have: require(<SomeString>)
				if (
				...
				{
					const identifier = declaration.id
					const requireCallArgument = declaration.init.arguments[0]
					...
					)
					{
						...
						let importPath = requireCallArgument.value
						assert(typeof importPath == 'string')
						
						// If the importPath is:
						//  1) Actually a path and not a pakage name
						//  2) Doesn't have an extension
						const importExtension = path.extname(importPath)
						if (importPath[0] == '.' && importExtension == '')
						{
							this.errors.push(`${scope} has an import without an extension: ${importPath}`)
							
							// We can safely assume that all require() calls are made to js files
						...
						const loc =
						{
							...
							// The end is after the end of the property in the MemberExpression
							end : ( declaration.init as WithLocation<EspreeAst.SimpleCallExpression> ).end
						}
			...
			// Otherwise it's something else we don't care about
		}
	...
	private createExportNode (
		node: EspreeAst.Directive | EspreeAst.Statement | EspreeAst.ModuleDeclaration,
		scope: string
	): ExportMetaAst| ReexportMetaAst | null
	{
		if (
		...
		{
			// Check that we have a "module.exports" assignment expression
			if (
			...
			)
			{
				const loc =
				{
					// The begining is the start of the whole ExpressionStatement
					start : ( node as WithLocation<EspreeAst.ExpressionStatement> ).start,
					
					// The end is after the end of the ExpressionStatement
					end : ( node as WithLocation<EspreeAst.ExpressionStatement> ).end
				}
				
				// If we have an Identifier, we're simply exporting a symbol
				if (node.expression.right.type == "Identifier")
				{
					const symbolId = node.expression.right.name
					
					const exportNode: ExportMetaAst =
					{
						type : 'ExportMetaAst',
						symbolId,
						loc
				...
				// If we have an ObjectExpression, we're exporting multiple symbols
				else if (node.expression.right.type == "ObjectExpression")
				{
					//debugger
				}
				// If we have an MemberExpression, we're either exporting a symbol OR re-exporting an import
				// (if the object part of the expression is a require() call, for instance)
				else if (node.expression.right.type == 'MemberExpression')  // <- Re-export
				{
					if (
						node.expression.right.object.type == 'CallExpression' &&
						node.expression.right.object.callee &&
						node.expression.right.object.callee.type == 'Identifier' &&
						node.expression.right.object.callee.name == 'require' &&
						node.expression.right.object.arguments[0] &&
						node.expression.right.object.arguments[0].type == 'Literal'
					)
					{
						const requireCallArgument = node.expression.right.object.arguments[0]
						const importPath = requireCallArgument.value
						assert(typeof importPath == 'string')
						
						const normalizedImportPath = importPath[0] == '.'
														? this.normalizedPath(importPath, scope)
														: importPath
						
						const property = node.expression.right.property;
						...
						const loc =
						{
							...
							// The end is after the end of the whole ExpressionStatement
							end : ( node as WithLocation<EspreeAst.ExpressionStatement> ).end
						}
						...
						return {
							...
							symbolId : property.name,
							loc
						}
					}
					else if (node.expression.right.object.type == 'Identifier')
					{
						//debugger
					}
					else
				...
				{
					// fixme: a bunch of cases we should probably handle	
					// module.exports = require("@platform/core").UndoRedoManager || require("@platform/core").UndoRedo;
				}
			}
